...
Quantidade de Faxinas por Dias da Semana - Gráfico de Barras
=====================================  
...
renderPlotly({
        ...
        g1a <- faxinas %>% filter(Mulher != "NA" & `Dia da Semana` != is.na(NA)) %>%
...
Quantidade de Faxinas por Dias da Semana - Boxplot
=====================================  

Row 
-------------------------------------

### Quantidade de Faxinas por dia da semana entre os anos 
```{r}
renderPlotly({
# Gráfico de Faxinas por dia da semana
# valor acumulado ao longo do periodo inserido para análise
b1 <- faxinas %>% filter(Mulher != "NA") %>% mutate(Quantidade = 1) %>%
  group_by(`Dia da Semana`) %>%
...
```{r}
### Boxplot de Faxinas por dias da Semana pelo Pureco por ano
### rever esse grafico
# renderPlotly({
# 
# b1a <-faxinas %>% filter(Mulher != "NA") %>% 
#   mutate(Quantidade = 1, ano = year(Data)) %>%
#   group_by(`Dia da Semana`) %>% 
#   mutate(Quantidade = sum(Quantidade)) %>%
#   ggplot(aes(x = `Dia da Semana`, y = Quantidade, fill = `Dia da Semana`)) +
#   geom_boxplot(stat = "identity") + facet_grid(~ano) +
#   ggtitle("Quantidade de Faxinas por Dia da Semana") + 
#   ylab("Quantidade de Faxinas") + 
#   theme(axis.text.x =  element_blank(),
#         axis.title.x = element_blank(),
...
```{r}
 renderPlotly({
        ...
        m1 <- ggplot(faxinas %>% mutate(Quantidade = 1, ano = year(Data)) %>%
