...
# como uso o ano em muitos graficos resolvi já mudar aqui 
faxinas <- faxinas %>% 
...
pickerInput(
  inputId = "grafico",
  label = "Escolha o tipo de calendário",
...
# imaginar o jeito melhor de fazer isso 
filtro_calendario <- reactive({
  ...
  disponibilidade2 %>% 
    ...
    filter(Disponibilidade != "NA" & Mulher %in% input$Mulher & ano %in% input$ano) %>%
    group_by(dia,Semana,ano,mes,`Dia da Semana`,Mulher) %>% 
    count(Mulher) 
})
...
renderPlotly({
 ...
 ggplotly(
    ggplot(filtro_calendario(), aes(y = `Dia da Semana`, x =  dia)) + 
    geom_tile(aes(fill = Mulher)) + 
    facet_wrap(ano~mes, scales = "free_x") +
...
#                           .(mesano),transform,semanadomes=1+Semana-min(Semana))  
  ...
  # geom_tile(aes_string(fill = input$mulher)) + 
