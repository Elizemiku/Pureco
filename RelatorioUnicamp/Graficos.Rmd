...
#                           .(mesano),transform,semanadomes=1+Semana-min(Semana))  
 ...
 ggplotly(disponibilidade %>% filter(Disponibilidade != "NA", Mulher == "Zilza") %>%
    mutate(ano = year(Data), mes = month(Data, label = TRUE),
           dia= day(Data),
           Semana = as.numeric(format(Data, "%W")),
           `Dia da Semana` = wday(Data, label=TRUE, abbr = FALSE,
                                  getOption("lubridate.week.start", 7))) %>% group_by(Semana,ano,mes,`Dia da Semana`,Mulher) %>% count(Mulher) %>% ggplot(aes(y = Semana, x = `Dia da Semana`)) + 
    geom_tile(aes(fill = Mulher)) + 
