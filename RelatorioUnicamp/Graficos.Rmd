...
# se quiser que coloca em ordem tem que ordenar as linhas e tirar os 
# Dia da Semana do eixo x porque o facets interfere na ordem   
# acrescentei a variavel ano como facets
  ...
  g1a <- faxinas %>%
    ...
    ggplot(aes(x = ordem, y = Quantidade, fill = `Dia da Semana`,
               text = paste("Quantidade:", Quantidade))) +
    geom_bar(stat = "identity", position = "stack") +
    facet_grid(~ano, scales = "free" , space = "free") + 
    scale_x_discrete("Dia da Semana", drop = FALSE) +
    scale_fill_viridis_d() +
    tema_facets 
...
renderPlotly({
  ...
  # mudar legenda desse para baixo  
f1 <- faxinas %>% 
