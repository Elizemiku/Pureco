...
<!--arquivo em flexdashboard-->
<!--argumento em HTML para deixar a página mais centralizada-->
<style>
body {
    position: center;
    left: 0px;}
...
```{r echo=FALSE}
## colocando opcoes dos chunks como echo=false, para nao mostrar os codigos apenas os resultados 
...
# opcoes de visualizacao do datatable
...
- Planilha de faxinas: período de 2018 a 2020. 
  ...
  - | **Nota feedback Mulher**: Nota que a colaboradora deu para o seu cliente. 
...
- Planilha de disponibilidade: período de 2018 a 2020.
  - | **Colaboradora** : Nome da colaboradora que tem disponibilidade para realizar a faxina.
...
- | Juntei as planilhas de Faxina do período de 2018 a 2019 e de 2019 a 2020 em apenas uma, e fiz o mesmo para as planilhas de disponibilidade.
...
- | Transformei os valores dos preços das faxinas em grupos de 20 em 20 reais (coluna **Valor**), como uma forma de dividir estes valores em grupos.
...
- | Gráfico da quantidade de faxinas por clientes novos - OK
...
### Planilha de faxinas do aplicativo :  Contém as informações das faxinas realizadas, dos clientes, das mulheres, comentários e feedbacks. 

```{r}
# lendo a planilha faxinas ja manipulada em Relatoriotecnicodados
# como a planilha se encontra na pasta www ele ja reconhece 
faxinas <- read_csv("faxinas.csv")

```

```{r echo=FALSE}
 
# este codigo cria uma tabela dinamica 
# resolvi remover pois qualquer um com link pode acessar os dados da tabela
...
### Planilha de disponibilidade das mulheres: Contém as informações dos dias disponíveis para realização das faxinas, para cada mulher que trabalha no PURECO. 
...
```{r, echo = FALSE}
# lendo a planilha de disponibilidade ja manipulada em Relatoriotecnicodados
# como a planilha se encontra na pasta www ele ja reconhece 
disponibilidade <- read_csv("disponibilidade.csv", col_names = TRUE)

```

```{r}
# tabela dinamica de disponibilidade
...
#     style = 'caption-side: top; text-align: left; color:blue; font-size: 16px',
#     'Tabela 2: ',
#     htmltools::em('Disponibilidade de dias das mulheres por ordem alfabética', )
#   ),
#   class = c('cell-border stripe'),
#   extensions = c('Buttons'),
#   options = list(dom = 'Bfrtip',
...
<!--A ideia inicial do projeto era tentar trabalhar com estas planilhas tambem -->

### Informações faxineiras : Contém as informações de todas as mulheres que trabalham no PURECO. 


### Informações Clientes : Contém as informações de todos os clientes do PURECO. 
