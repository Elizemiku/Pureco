...
- Mudanças:
  ...
  - | Na parte de limpeza dos dados, foram removidas todas as colunas vazias ou que continham números sem sentido. Os valores: 0 ou 1, true ou false, foram transformados para sim ou não.
...
faxinas201819 <- faxinas201819 %>%
  ...
  mutate(Data = convert_to_datetime(Data, character_fun = lubridate::mdy, tz = "UTC")) %>%
  ...
  select(c(1:11)) %>% rename(Comentários = "...11") %>%
  remove_empty("rows")
...
faxinas <- full_join(faxinas201819, faxinas201920, 
                     by = c("Data", "Mulher", "Valor", "Cliente", "Endereço", "Ocorreu?", "Feedback Colhido?",
                            "Onde foi colhido?", "Feedback cliente", "Comentários"))
```
