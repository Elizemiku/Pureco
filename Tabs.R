### Tabs.R é o script de código que contem todas opções de personalização das abas do site ### 
## do ui.R, (seria como um front-end para o site)
## decidi criar este código separado devido ao tamanho do código
## o ui.R chama todas abas e páginas deste código


# codigo com as modificacoes dos dados de faxinas
source("faxinas.R")
# carregando dados da planilha de faxinas 
faxinas <- carregando_dados_f()

# Tabelas dinâmicas separadas
## Primeira parte comentada para entender melhor como funciona cada função

# Aba de Inicio

## tabPanel cria uma aba dentro da pagina
## as opcoes style sao provenientes das opcoes de html 
Tab1 <- tabPanel(
  # nome da aba
  title = "Início",
  # icone escolhido
  icon = icon("home"),
  # layout da pagina fluido
  fluid = TRUE,
  # Mensagem ao abrir esta aba
  titlePanel(
    h1("Seja Bem-Vindo(a)!",
       # estilo e alinhamento do texto
  ...
  fluidPage(# Barra lateral com as definições do input e do output.
    sidebarLayout(
      ...
      sidebarPanel(
        ...
        # actionButton e um botao que gera alguma ação ao clicar
        # neste caso ao clicar em OK a imagem do pureco aparece quando o htmlOutput inicio
        # é chamado abaixo diretamente do server.R 
        actionButton(
          inputId = "botao",
          label = "OK!",
...
## icon mostra o icone de grafico de barras
          ...
          tabPanel(title = "Informações Gerais das Faxinas",
            ...
            sidebarLayout(
              sidebarPanel(
                ...
                ## gráficos
                ## pickerInput é como um selectInput com mais opções de personalização 
                ## este input oferece uma lista de opções ao usuário 
                pickerInput(
                  inputId = "grafico",
                ...
                ## opcoes do gráfico para aparecer no eixo x 
                pickerInput(
                  inputId = "eixo_x",
                ...
                ## opcoes do gráfico para aparecer no eixo y
                pickerInput(
                  inputId = "eixo_y",
                ...
                ## opcoes do grafico para aparecer como um grupo (fill) no gráfico
                pickerInput(
                  inputId = "grupo",
                  label = "Selecione uma opção adicional:",
                  choices = c("Nenhum","Ocorreu?", "Região", "Remarcou", "Tipo", "Valor"), 
                  # quando Nenhum for selecionado não há nenhum grupo no gráfico
                  selected = "Nenhum",
                  multiple = FALSE,
                  options = list(title='Escolha uma opção:',
                                 style = "color: black; background: white; font-weight: bold;")),
                
              ## botão de ação que aparece os gráficos depois das escolhas das opções
              actionButton(
                inputId = "escolhido",
                label = "Gerar Gráfico",
                style = "color: #fff; background-color: #337ab7; border-color: #2e6da4")
             ...
             ## painel onde ficam os gráficos usar função plotlyOutput, pois os gráficos são plotly
             mainPanel(title = "Gráfico", 
                      # infgeral1parte1 é o nome do output do server.R
      ...
      tabPanel(title = "Informações das Colaboradoras",
               ...
               sidebarLayout(
                 sidebarPanel(
                   ...
                   ## variavel da escolha de colaboradora para analisar nos gráficos
                   pickerInput(
                     inputId = "mulher",
                     label = "Caso queira ver outras análises por colaboradora selecione uma ou mais mulheres:",
                     # pegando em ordem alfabetica as colaboradoras dos dados de faxinas
                     choices = sort(unique(faxinas$Colaboradora)),
                     # selecionando todas as colaboradoras escolhidas
                 ...
                 ## painel onde ficam os gráficos da seção de informações de colaboradoras 
                  mainPanel(title = "Gráfico",   
        ...
        tabPanel(title = "Disponibilidade das Colaboradoras",
                 ...
                 fluid = TRUE,
                       ...
                       sidebarLayout(
                         sidebarPanel(
                           ...
                           radioButtons(
                             ...
                             ## pegando todas as colaboradoras em ordem alfabetica dos dados de faxinas e 
                             ## selecionando apenas a Lourdes como primeira opção
                             choices = sort(unique(faxinas$Colaboradora)),
                             selected = "Lourdes",
                             inline = FALSE),
                       ...
                       ## painel onde ficam os gráficos da seção de calendário
        ...
        tabPanel(title = "Informações dos Clientes",
                 ...
                 sidebarLayout(
                   sidebarPanel(
                     ...
                     ## ano (nao coloquei 2018 pois na planilha de faxinas de 2018, nao tem a variavel tipo)
                     pickerInput(
                       inputId = "ano_c",
                       label = "Selecione o ano que deseja:",
                     ...
                     ## opcoes do gráfico para aparecer no eixo x, eixo y aqui é sempre "Quantidade"
                     pickerInput(
                       inputId = "eixo_x_c",
                       label = "Selecione o tipo de ocorrência que deseja analisar:",
                       choices = c("Dia da Semana" = "Semana", "Mês"),
...
# Aba de Tutorial
# metade dela e feita aqui e metade aparece no server.R (a parte do mainPanel)
Tab4 <- tabPanel(
  "Tutorial",
  ...
  ## mini página fluida no qual aparece indicações sobre o site 
  fluidPage(sidebarLayout(
    ...
    ## no meio da página aparece informações depois que clica ok no primeiro botão da página de ínicio
    mainPanel(uiOutput("tutorial"))
  ))
)

# Aba com informações sobre o site 
Tab5 <- tabPanel("Sobre",
                 icon = icon("info-circle"), 
                 fluid = TRUE,
                 ## pedaço de página fluida 
                 ## dentro desta aba com uma
                 ## coluna de tamanho 6
                 fluidRow(
                   column(6,
                          ## h4 e h5 seguem os padroes de tamanho de letra do html 
                          ## tags$strong() deixa a frase em negrito
                          ## href referencia um site 
                          ## src e um source de uma imagem no caso o link do google de onde vem esta imagem 
